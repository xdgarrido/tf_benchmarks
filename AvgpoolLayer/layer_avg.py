...
def layer_avg(x, pool_size=(2, 2), strides=(1,1), name=None):
  """Run average pool on the input tensor."""
  return(tf.keras.layers.AveragePooling2D(pool_size, strides=None, padding='valid', data_format='channels_last')(x))
  
class AvgLayer(tf.test.TestCase):
 
  def test_avglayer(self):
    ...
    context_layer_gpu  = layer_avg(x_trf, pool_size, strides)
