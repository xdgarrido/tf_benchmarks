...
for BATCH in  16 32 64 128 256 512 
  do 
    for SIZE in  32 64 128 256 512 1024 2048 4096 
     do 
                echo $BATCH:$SIZE:$PRECISION
                echo "[LAYERNORM]"  >> $SCRIPTPATH/eval_results.txt
                ...
                starttime=$(date +%s)
                # run dense layer
                docker exec $CTNRNAME python3 $CODE_DIR_INSIDE/tf_benchmarks/NormLayer/layer_normalization.py --iter=$COUNT --precision=$PRECISION --mode=$MODE --batch_size=$BATCH --hidden_size=$SIZE 2>&1 | tee $SCRIPTPATH/NormLayer/log.txt
                endtime=$(date +%s)
